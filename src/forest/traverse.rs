...
use super::Bocage;
use super::node::{NodeHandle, Node, Iter, Tag};
use super::node::Node::*;
    ...
    where G: Borrow<InternalGrammar>,
{
    pub fn next_node<'t>(&'t mut self) -> Option<TraversalHandle<'f, 't, G>> {
        while let Some(node) = self.graph_iter.peek() {
            ...
            println!("next_node @{:?} {:?} {}", self.graph_iter.handle, node, alive);
    ...
    fn unfold_factors(&mut self, left: NodeHandle, right: Option<NodeHandle>) {
        ...
        while let Some(node) = self.pop_for_unfold() {
            match node {
                (Product { left_factor, right_factor, .. }, _) => {
    ...
    fn pop_for_unfold(&mut self) -> Option<(Node, NodeHandle)> {
        self.factor_traversal.pop().map(|handle| {
            (self.bocage.graph.get(handle), handle)
        })
    }
}
