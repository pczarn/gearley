...
use gearley_grammar::{EventAndDistance, ExternalDottedRule, Grammar};
    ...
    P: PerfHint,
{
    pub fn trace(&self) -> impl Iterator<Item = (ExternalDottedRule, usize)> + use<'_, F, G, P> {
        ...
        self.predicted_symbols()
            .map(|sym| (trace_predict[sym.usize()], self.earleme()))
            .chain(
                self.medial_items()
                    .map(|item| (trace0[item.dot as usize], item.origin as usize)),
            )
    }
    ...
    pub fn events(&self) -> impl Iterator<Item = EventAndDistance> + use<'_, F, G, P> {
        ...
        self.predicted_symbols()
            .map(|sym| events_predict[sym.usize()])
            .chain(
                self.medial_items()
                    .map(|item| events_flat[item.dot as usize]),
            )
    }
    ...
    pub fn expected_terminals(&self) -> impl Iterator<Item = Symbol> + use<'_, F, G, P> {
        ...
        self.medial_items()
            .map(|item| rhs1[item.dot as usize].unwrap())
    }
}
